% A distributed fusion estimation approach based on the robust finite horizon filtering with packet disorders
function [sigma1, Trtheta1, P1, x3, M1, M2, fv, et]=RFHDFE_ZOH_Function_TCYB(T, A, B, C, E, F1, H1, F, a, beta,...
...
for k=1:iter
    ...
    if s1(k,1)>=0 && s1(k,1)<=1
        if s1(k,1)==1
            ...
            Trtheta1(t1(k,1),1)=trace(theta1(:,3*t1(k,1)-2:3*t1(k,1)));             % Trace(P1);
